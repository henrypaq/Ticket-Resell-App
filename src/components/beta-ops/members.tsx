...
export function AddMemberForm() {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3 rounded-[18px] border border-hairline bg-white/[0.04] p-4">
      <p className="text-[14px] font-semibold text-ink">Add member</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Source" htmlFor="add-source">
          <select id="add-source" name="acquisitionChannel" defaultValue="manual" className={FIELD_CLASS}>
      ...
      <Field label="Internal notes (optional)" htmlFor="add-notes">
        <input id="add-notes" name="notes" className={FIELD_CLASS} placeholder="Context for the team" />
      </Field>
      {state.error && (
      ...
      {state.ok && <p className="text-[13px] text-muted">Added.</p>}
